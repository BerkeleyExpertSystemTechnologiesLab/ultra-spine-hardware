...
void loop() {
  ...
  delay(delayMotion);  
    StopMotors();
  LightFunction();   
  Motor3(true,200);  //Motor3 rotates for 1 second CW
  delay(delayMotion); 
  StopMotors();      //Motor stops
  LightFunction();   
  Motor3(false,200); //Motor3 rotates for 1 second CCW
  delay(delayMotion);  
  StopMotors(); 
  LightFunction();
  Motor1(false,200); //Motor1 rotates for 1 second CW
  delay(delayMotion);
  StopMotors();
  LightFunction();
  Motor1(true,200);  //Motor1 rotates for 1 second CCW
  delay(delayMotion);  
}
